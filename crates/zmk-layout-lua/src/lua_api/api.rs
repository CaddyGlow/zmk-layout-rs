...
/// Install the layout API as a global `layout` variable.
...
fn load_manifest_for_lua(path: &str) -> LuaResult<zmk_layout_core::build::FirmwareManifest> {
    use std::path::Path;
...
fn build_request_from_table(
    ...
    let progress: Arc<dyn zmk_layout_core::build::progress::ProgressReporter> = if verbose {
